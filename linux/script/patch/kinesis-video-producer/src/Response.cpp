...
shared_ptr<Response> Response::create(Request &request) {
    ...
    char * CURLOPT_SSL_VERIFYPEER_str = getenv("CURLOPT_SSL_VERIFYPEER");
    if(CURLOPT_SSL_VERIFYPEER_str != NULL && strlen(CURLOPT_SSL_VERIFYPEER_str) > 0)
    {
        curl_easy_setopt(response->curl_,CURLOPT_SSL_VERIFYPEER ,atoi(CURLOPT_SSL_VERIFYPEER_str) == 1?true:false);
    }
    char * CURL_CA_BUNDLE_str = getenv("CURL_CA_BUNDLE");
    if(CURL_CA_BUNDLE_str != NULL && strlen(CURL_CA_BUNDLE_str) > 0)
    {
        curl_easy_setopt(response->curl_,CURLOPT_CAINFO ,CURL_CA_BUNDLE_str);
    }
    char * SSL_CERT_DIR_str = getenv("SSL_CERT_DIR");
    if(SSL_CERT_DIR_str != NULL && strlen(SSL_CERT_DIR_str) > 0)
    {
        curl_easy_setopt(response->curl_,CURLOPT_CAPATH ,SSL_CERT_DIR_str);
    }
    char * SSL_CERT_FILE_str = getenv("SSL_CERT_FILE");
    if(SSL_CERT_FILE_str != NULL && strlen(SSL_CERT_FILE_str) > 0)
    {
        curl_easy_setopt(response->curl_,CURLOPT_CAINFO ,SSL_CERT_FILE_str);
    }
